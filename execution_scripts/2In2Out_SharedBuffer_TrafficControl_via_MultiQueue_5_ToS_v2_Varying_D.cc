...
int main (int argc, char *argv[])
{
  ...
  for (size_t i = 0; i < arrayLength; i++) // iterate over all D values to create SourceAppContainers Array
  {
  ...
  std::vector<double_t> Num_M_vector;  
  std::vector<bool> unequalNum_vector;  
  for (size_t i = 0; i < arrayLength; i++) // iterate over all D values to create SourceAppContainers Array
  {
    double_t Num_M = miceElephantProb_array[i] * 10;  // number of OnOff machines that generate High priority traffic
    Num_M_vector.push_back(Num_M);
  ...
  for (size_t i = 0; i < 2; i++)
  {
    ...
    for (size_t j = 0; j < arrayLength; j++) // iterate over all D values to configure and install OnOff Application for each Client
    {
      ...
      if (applicationType.compare("prioOnOff") == 0) 
      {
        for (size_t k = 0; k < 5; k++) 
        {
          ...
          if (unequalNum_vector[j]) 
          {
            ...
            if (i == 0 && j < int(ceil(Num_M_vector[j]/2)) || i == 1 && j < int(floor(Num_M_vector[j]/2)))
            {
          ...
          {
            if (j < int(Num_M_vector[j]/2)) // create OnOff machines that generate High priority traffic
            {
              if (onOffTrafficMode.compare("Constant") == 0)
              { 
                clientHelpers_matrix[j][k].SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(miceOnTime) + "]"));
  ...
  for (size_t i = 0; i < arrayLength; i++) // iterate over all D values to synchronize Start/Stop for all OnOffApps
  {
  ...
  std::string dirToSave = dir + traffic_control_type + "/" + implementation + "/" + onOffTrafficMode + "/";
  ...
  if (adjustableAlphas)
  {
    ...
    // dirToSave = dir + traffic_control_type + "/" + implementation + "/" + onOffTrafficMode + "/"
    std::string newDir = dirToSave + "VaryingDValues/adjustableAlphas/";
    system (("mkdir -p " + newDir).c_str ());
    system (("mv -f " + dir + traffic_control_type + "/" + implementation + "/" "*.dat " + newDir).c_str ());
