...
// There is 1 server1 connecting to a switch
...
// trace TCP Tx Congestion Window
void
CwndChange(uint32_t oldCwnd, uint32_t newCwnd)
{
  // NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "\t" << newCwnd);
  std::cout << "CwndChange called: " << Simulator::Now().GetSeconds() << " " << newCwnd << std::endl;
  // save to .dat file
  std::ofstream cwndChangeTrace (dir + traffic_control_type + "/" + implementation + "/TxCongestionWindowChange.dat", std::ios::out | std::ios::app);
  cwndChangeTrace << Simulator::Now ().GetSeconds () << " " << newCwnd << std::endl;
  cwndChangeTrace.close ();
}
...
{
  LogComponentEnable ("Line", LOG_LEVEL_INFO);
  ...
  else
  {
    ...
    queue_capacity = ToString(BUFFER_SIZE/2) + "p"; // B, the total space on the buffer [packets]
  }
  ...
  if (transportProt.compare ("TCP") == 0)
  {
    ...
    // Set default initial congestion window as 100 segments
  ...
  // s2r.DisableFlowControl();

  NS_LOG_INFO ("Create NetDevices");
  NetDeviceContainer serverDevice;
  NetDeviceContainer switchDeviceIn;
  NetDeviceContainer switchDeviceOut;
  ...
  for (int i = 0; i < SERVER_COUNT; i++)
  {
    ...
    NetDeviceContainer tempNetDevice = n2s.Install (server.Get (i), router.Get(0));
    serverDevice.Add(tempNetDevice.Get(0));
    switchDeviceIn.Add(tempNetDevice.Get(1));
  }
  ...
  {
    NetDeviceContainer tempNetDevice = s2r.Install (router.Get(0), reciever.Get (i));
    switchDeviceOut.Add(tempNetDevice.Get(0));
    recieverDevice.Add(tempNetDevice.Get(1));

    NS_LOG_INFO ("Switch is connected to Reciever " << i << "at capacity: " << switchRecieverCapacity);        
  }

  for (size_t i = 0; i < switchDeviceOut.GetN(); i++)
  {     
    Names::Add("switchDeviceOut" + IntToString(i), switchDeviceOut.Get(i));  // Add a Name to the switch net-devices
  }
  ...
  QueueDiscContainer qdiscs = tch.Install (switchDeviceOut);  // in this option we installed TCH on switchDevicesOut. to send data from switch to reciever
  ...
  for (int i = 0; i < SERVER_COUNT; i++)
  {
    ...
    tempNetDevice.Add(serverDevice.Get(i));
    tempNetDevice.Add(switchDeviceIn.Get(i));
  ...
  {
    NetDeviceContainer tempNetDevice;
    tempNetDevice.Add(switchDeviceOut.Get(i));
    tempNetDevice.Add(recieverDevice.Get(i));
  ...
  for (size_t i = 0; i < SERVER_COUNT; i++)
  {
    ...
    if (transportProt.compare("TCP") == 0) 
    {
      // setup source socket
      sockptr =
          ns3::Socket::CreateSocket(server.Get(serverIndex),
                  ns3::TcpSocketFactory::GetTypeId());

      sockptr->TraceConnectWithoutContext("CongestionWindow", MakeCallback(&CwndChange));
    ...
    if (applicationType.compare("standardClient") == 0) 
    {
      ...
      sourceApps.Add(clientHelperP0.Install (server.Get(serverIndex)));
      ...
      // sourceApps.Add(clientHelperP1.Install (server.Get(serverIndex)));
    } 
    ...
    sinkApps.Add(sinkP0.Install (reciever.Get(recieverIndex)));
