 ...
 */
#include "udp-prio-client-server-helper.h"
...
UdpPrioClientHelper::UdpPrioClientHelper()
{
    m_factory.SetTypeId(UdpPrioClient::GetTypeId());
}

UdpPrioClientHelper::UdpPrioClientHelper(Address address, uint16_t port)
{
...
UdpPrioClientHelper::UdpPrioClientHelper(Address address)
{
    m_factory.SetTypeId(UdpPrioClient::GetTypeId());
    SetAttribute("RemoteAddress", AddressValue(address));
}
...
void
UdpPrioClientHelper::SetAttribute(std::string name, const AttributeValue& value)
{
    m_factory.Set(name, value);
}

ApplicationContainer
UdpPrioClientHelper::Install(NodeContainer c)
{
    ApplicationContainer apps;
    for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i)
    {
        Ptr<Node> node = *i;
        Ptr<UdpPrioClient> client = m_factory.Create<UdpPrioClient>();
