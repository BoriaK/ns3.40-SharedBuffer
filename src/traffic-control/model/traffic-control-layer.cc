            ...
            if (nodeName.find("Router")==0)
            {
                ...
                if (m_flow_priority == 1)
                {
                    ...
                    else if (m_usedAlgorythm.compare("FB") == 0)
                    {
                        ...
                        if (internal_qDisc->GetNumOfHighPrioPacketsInQueue().GetValue() < GetQueueThreshold_FB_v2(m_alpha, m_alpha_l, m_alpha_h, conjestedQueues, gamma_i).GetValue())
                        {
                            ...
                            std::cout << "number of High Priority packets in queue: " << internal_qDisc->GetNumOfHighPrioPacketsInQueue().GetValue() << std::endl;
                            std::cout << "High Priority packet threshold is: " << GetQueueThreshold_FB_v2(m_alpha, m_alpha_l, m_alpha_h, conjestedQueues, gamma_i).GetValue() << std::endl;
                ...
                else
                {
                    ...
                    else if (m_usedAlgorythm.compare("FB") == 0)
                    {
                        ...
                        {
                            // for debug loop issue:
                            std::cout << "number of Low Priority packets in queue: " << internal_qDisc->GetNumOfLowPrioPacketsInQueue().GetValue() << std::endl;
                            std::cout << "Low Priority packet threshold is: " << GetQueueThreshold_FB_v2(m_alpha, m_alpha_l, m_alpha_h, conjestedQueues, gamma_i).GetValue() << std::endl;
