...
static std::string g_outputDirectory = ".";  // Default to current directory
    ...
    static TypeId tid =
        TypeId("ns3::TrafficControlLayer")
            ...
            .AddAttribute("Alpha_Low",
                          ...
                          DoubleValue(1),
                          MakeDoubleAccessor(&TrafficControlLayer::m_pre_alpha_l),
                          MakeDoubleChecker<double_t>())
...
// In traffic-control-layer.cc
void
TrafficControlLayer::SetGlobalOutputDirectory(const std::string& dir)
{
    g_outputDirectory = dir;  // Updates the global variable
}
...
float_t
TrafficControlLayer::GetNormalizedDequeueBandWidth_v1_transient(Ptr<NetDevice> device, uint16_t queueIndex)
{
    NS_LOG_FUNCTION(this);
...
TrafficControlLayer::GetQueueThreshold_DT(double_t alpha, double_t alpha_l, double_t alpha_h) // added by me!!!!!!!! for DT implementation
{
    ...
    // DEBUG_FIX_CLAMPING: Prevent uint32_t underflow in buffer calculation
    if (GetMaxSharedBufferSize().GetUnit() == QueueSizeUnit::PACKETS)
    {
    ...
    if (GetMaxSharedBufferSize().GetUnit() == QueueSizeUnit::PACKETS)
    {
        ...
        else if (alpha == alpha_l)
        {
            if (!m_freezeLowPriorityThreshold) // we enter transient state
            {
                ...
                {
                    float m_gamma_i = GetNormalizedDequeueBandWidth_v1_transient(device, subQueueIndex);
                            // for debug:
            ...
            else if (!m_inTransient && GetNumOfHighPriorityPacketsInSharedQueue().GetValue() == 0) // if the transient has passed
            {
                m_p_threshold_l = alpha_l * remainingSpace;
                m_freezeLowPriorityThreshold = false; // unfreeze the low priority threshold calculation

                // release "let all packets" flag
                m_flag = false;
            }
            else // if we're in the middle of the transient
            {
                ...
                // m_p_threshold_l = std::max(storeLowPriorityThreshold, GetQueueThreshold_DT(m_alpha, m_alpha_l, m_alpha_h).GetValue());
                m_p_threshold_l = storedLowPriorityThreshold;
...
TrafficControlLayer::GetQueueThreshold_FB(double_t alpha, double_t alpha_l, double_t alpha_h, int conjestedQueues, float gamma) // for FB implementation. v2 excepts Np(t) as an external variable
{
        ...
        0) // if queue is compleatly empty -> Threshold should be as high as possible (Infinity)
    {
        ...
        if (GetMaxSharedBufferSize().GetUnit() == QueueSizeUnit::PACKETS)
        {
            if (alpha == alpha_h)
            {
                m_p_threshold_h = alpha_h * m_gamma * remainingSpace;
                return QueueSize(QueueSizeUnit::PACKETS, m_p_threshold_h);
            }
            else if (alpha == alpha_l)
            {
                m_p_threshold_l = alpha_l * m_gamma * remainingSpace;
                return QueueSize(QueueSizeUnit::PACKETS, m_p_threshold_l);
            }
        }
        if (GetMaxSharedBufferSize().GetUnit() == QueueSizeUnit::BYTES)
        {
            if (alpha == alpha_h)
            {
                m_b_threshold_h = alpha_h * m_gamma * remainingSpace;
                return QueueSize(QueueSizeUnit::PACKETS, m_b_threshold_h);
            }
            else if (alpha == alpha_l)
            {
    ...
    else // some queues are already congested
    {
        ...
        if (GetMaxSharedBufferSize().GetUnit() == QueueSizeUnit::PACKETS)
        {
            ...
            if (alpha == alpha_h)
            {
                ...
                m_p_threshold_h =
                    alpha_h * (1 / m_numConjestedQueuesHigh) * m_gamma * remainingSpace;
                return QueueSize(QueueSizeUnit::PACKETS, m_p_threshold_h);
            }
            else if (alpha == alpha_l)
            {
                ...
                m_p_threshold_l =
                    alpha_l * (1 / m_numConjestedQueuesLow) * m_gamma * remainingSpace;
                return QueueSize(QueueSizeUnit::PACKETS, m_p_threshold_l);
            }
        }
        if (GetMaxSharedBufferSize().GetUnit() == QueueSizeUnit::BYTES)
        {
            ...
            {
                m_numConjestedQueuesHigh = conjestedQueues;
                m_b_threshold_h = std::ceil(alpha_h * (1 / m_numConjestedQueuesHigh) * m_gamma * remainingSpace);
                return QueueSize(QueueSizeUnit::PACKETS, m_b_threshold_h);
            }
            else if (alpha == alpha_l)
            {
                m_numConjestedQueuesLow = conjestedQueues;
                m_p_threshold_l = std::ceil(alpha_l * (1 / m_numConjestedQueuesLow) * m_gamma * remainingSpace);
                return QueueSize(QueueSizeUnit::PACKETS, m_b_threshold_l);
            }
        }
        NS_ABORT_MSG("Unknown Threshod unit");
...
{   
    ...
    // make sure that Run-Time (for Predictive model) that has passed is > Tau
    // since the predictive model starts to run at t0 - Tau/2, the last packet arrival time should be at least t0 + Tau/2.
    if ((lastRow.time - (1 + m_Tau/2)*1e+6) > 0)
    {
        // take the last row of the dataset and subtract Tau from it to get the start time of the window
        // then take the row that corresponds to the start time of the window
        int64_t windowStartTime = lastRow.time - m_Tau*1e+6;
        currentRow = dataset.GetRow(windowStartTime);
        std::cout << "winow length [uSec]: " << lastRow.time - currentRow.time << std::endl;
    }
    else  // if Time (for Predictive model) that has passed is <= Tau, meaning that the real model hasn't been running long enough
          // take the maximum window avalible
    {
    ...
    // std::cout << "Total sum of packets: " << currentRow.var2 << std::endl;
    // std::cout << "Sum of high priority packets: " << currentRow.var3 << std::endl;
    // std::cout << "Sum of low priority packets: " << currentRow.var4 << std::endl;

    // std::cout << "Window end time: " << lastRow.time << std::endl;
    // std::cout << "Total sum of packets: " << lastRow.var2 << std::endl;
    // std::cout << "Sum of high priority packets " << lastRow.var3 << std::endl;
    // std::cout << "Sum of low priority packets: " << lastRow.var4 << std::endl;

    m_predictedArrivingTraffic = lastRow.var2 - currentRow.var2;
    m_predictedArrivingHighPriorityTraffic = lastRow.var3 - currentRow.var3;
    ...
    std::ofstream estimatedDValuesOutputFile(g_outputDirectory + "/Estimated_D_Values.dat", std::ios::app); 
                ...
                // " " << GetQueueThreshold_DT(m_alpha_l, m_alpha_l, m_alpha_h).GetValue() <<
...
TrafficControlLayer::GetDeltaAlphas(uint32_t incoming_transient_lentgh)
{
    ...
    //     m_deltaAlphas = 0;
    //     break;
    // case 12288:  // corresponds to 50[mSec]
    //     m_deltaAlphas = 0;  // best for initial alphas 15-5
    //     break;
    // case 61440:  // corresponds to 100[mSec]
    //     m_deltaAlphas = 9;
        break;
    default:
        break;
...
TrafficControlLayer::DropBeforeEnqueue(Ptr<const QueueDiscItem> item)
{
    ...
    // Build the full path using the global directory
    std::string filename = g_outputDirectory + "/List_Predictive_Packets_Dropped_by_SharedBuffer.dat";
    
    if (m_stats.nTotalDroppedPackets == 0 && nodeName.compare("RouterPredict") == 0)
    {
        std::remove(filename.c_str());  // Deletes from the correct directory
    }
    ...
    if (nodeName.compare("RouterPredict") == 0)
    {
        ...
        std::ofstream listPredictedPacketsDroppedFile(filename, std::ios::app);
...
TrafficControlLayer::DequeueCallback(Ptr<const QueueDiscItem> item)
{
    ...
    // Threshold updates during dequeue have been removed
    // Thresholds are now only calculated during enqueue operations
    // This avoids race conditions where GetCurrentSharedBufferSize() 
    // returns inconsistent values during distributed queue updates
}
...
TrafficControlLayer::Send(Ptr<NetDevice> device, Ptr<QueueDiscItem> item)
{
    ...
    // Calculate port index early for use in SetTxQueueIndex throughout Send()
    // This extracts the port number from device names like "switchDeviceOut0" or "switchDeviceOut1"
    std::string netDeviceName = Names::FindName(device);
    ...
    // load the Alpha High/Low values from predefined. this is needed because predictive model updates alphas and it needed to 
    // restore them for next packet processing
    m_alpha_h = m_pre_alpha_h;
    m_alpha_l = m_pre_alpha_l;
        ...
        if (m_useSharedBuffer)
        {
            ...
            if (nodeName.find("Router") == 0 && (tcpHeader.GetSourcePort() != 50000 && tcpHeader.GetSourcePort() != 50001 && tcpHeader.GetSourcePort() != 50002 &&  tcpHeader.GetSourcePort() != 50003 &&  tcpHeader.GetSourcePort() != 50004) && (item->GetPacket()->PeekPacketTag(flowPrioTag)))
            {
                ...
                if (isUdp || (item->GetPacket()->GetSize() >= 1024)) // pass only UDP packets or TCP Data packets. TCP Control Packets are less than 1024 [Bytes]
                {
                    ...
                    uint16_t realOnOffApplicationTos = 0;
                    int destinationPort = tcpHeader.GetDestinationPort();
                    uint16_t m_subQueueIndex = m_tosPrioMap[0];
                    if (ndi->second.m_rootQueueDisc->GetNQueueDiscClasses() > 1) // if we use multiple queues/port.
                    {
                        // uint16_t m_subQueueIndex = m_tosPrioMap[0];
                        // SocketIpTosTag ipTosTag;
                        ...
                        {
                            //for debug:
                            // std::cout << "packet: " << item->GetPacket() << " IP ToS : " << int(ipTosTag.GetTos()) << std::endl;
                            ...
                            {
                            case 50000:
                                realOnOffApplicationTos = 0x10;
                                break;
                            case 50001:
                                realOnOffApplicationTos = 0x6;
                                break;
                            case 50002:
                                realOnOffApplicationTos = 0x4;
                                break;
                            case 50003:
                                realOnOffApplicationTos = 0x2;
                                break;
                            case 50004:
                                realOnOffApplicationTos = 0x0;
                                break;
                            default:
                                break;
                            }
                            ...
                            m_subQueueIndex = m_tosPrioMap[(ipTosTag.GetTos() / 2) & 0x0f];
                        }
                        ...
                        ///////////////////////////////////////
                        internal_qDisc = qDisc->GetQueueDiscClass(m_subQueueIndex)->GetQueueDisc();
                        NS_ASSERT(internal_qDisc);
                        m_multiQueuePerPort = true;
                    }
                    else
                    {
                        m_subQueueIndex = 0; // efectively it doesn't matter
                        // for debug:
                        if (item->GetPacket()->PeekPacketTag(ipTosTag))
                        {
                    ...
                    if (m_usedAlgorythm.compare("DT") == 0 || m_usedAlgorythm.compare("PredictiveDT") == 0)
                    {
                        ...
                        {                            
                            m_alpha = m_alpha_h;
                            m_queueThresholdDT = GetQueueThreshold_DT(m_alpha, m_alpha_l, m_alpha_h).GetValue();
