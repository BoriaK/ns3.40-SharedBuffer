...
viaMQueues2ToS (std::string traffic_control_type, double_t alpha_high, double_t alpha_low, bool accumulateStats)
{
  ...
  // move all the produced files to a directory based on the Alpha values
  std::string newDir = dirToSave + traffic_control_type + "/" + implementation + "/" + DoubleToString(miceElephantProb) + "/" 
                      + DoubleToString(alpha_high) + "_" + DoubleToString(alpha_low) + "/";
  system (("mkdir -p " + newDir).c_str ());
  system (("mv -f " + dirToSave + "/*.dat " + newDir).c_str ());
  system (("mv -f " + dirToSave + "/*.txt " + newDir).c_str ());
  ...
  if (accumulateStats)
  {
    if (!(std::filesystem::exists(dirToSave + "/TestStats/" + implementation + "/" + DoubleToString(miceElephantProb) + "/" 
                                  + usedAlgorythm + "_TestAccumulativeStatistics.dat")))
    {
      // If the file doesn't exist, create it and write initial statistics
      system (("mkdir -p " + dirToSave + "/TestStats/" + implementation + "/" + DoubleToString(miceElephantProb) + "/").c_str ());
      std::ofstream testAccumulativeStats (dirToSave + "/TestStats/" + implementation + "/" + DoubleToString(miceElephantProb) + "/" 
    ...
    {
      // Open the file in append mode
      std::fstream testAccumulativeStats (dirToSave + "/TestStats/" + implementation + "/" + DoubleToString(miceElephantProb) + "/" 
                                          + usedAlgorythm + "_TestAccumulativeStatistics.dat", std::ios::app);
      testAccumulativeStats
      << DoubleToString(alpha_high) + "_" + DoubleToString(alpha_low) << " "
      << tcStats.nTotalDroppedPackets << " " 
...
viaMQueues2ToSVaryingD (std::string traffic_control_type, double_t alpha_high, double_t alpha_low, bool accumulateStats)
{
  ...
  for (size_t i = 0; i < 2; i++)
  {
      ...
      if (applicationType.compare("prioOnOff") == 0) 
      {
        ...
        // for d = 0.5
        miceElephantProb = 0.5; // [0.1, 0.9] the probability to generate mice compared to elephant packets. 
        // in this simulation it effects silence time ratio for the onoff application
        ...
        // for d = 0.7
        miceElephantProb = 0.7; // [0.1, 0.9] the probability to generate mice compared to elephant packets. 
        // in this simulation it effects silence time ratio for the onoff application
        miceOffTime = DoubleToString((1 - miceElephantProb) * 2 * miceOffTimeConst); // [sec]
        elephantOffTime = DoubleToString(miceElephantProb * 2 * elephantOffTimeConst); // [sec]
