    ...
    if (applicationType.compare("prioOnOff") == 0) 
    {
      for (size_t j = 0; j < 5; j++) 
      {
        ...
        if (unequalNum) 
        {
          ...
          if (((i == 0) && (j < static_cast<size_t>(ceil(Num_M_High/2)))) || ((i == 1) && (j < static_cast<size_t>(floor(Num_M_High/2)))))
          {
            if (onoff_traffic_mode.compare("Constant") == 0)
            { 
        ...
        {
          if (j < size_t(Num_M_High/2)) // create OnOff machines that generate High priority traffic
          {
            if (onoff_traffic_mode.compare("Constant") == 0)
            { 
              clientHelpers_vector[j].SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(miceOnTime) + "]"));
      ...
      if (applicationType.compare("prioOnOff") == 0) 
      {
        for (size_t k = 0; k < 5; k++) 
        {
          ...
          if (unequalNum_vector[j]) 
          {
            ...
            // 1st server recives the higher number of High priority and lower number of Low priority OnOff applications
            // and the 2nd server recives lower number of High priority and higher number of Low priority OnOff applications
            if (((i == 0) && (k < static_cast<size_t>(ceil(Num_M_High_vector[j]/2)))) || ((i == 1) && (k < static_cast<size_t>(floor(Num_M_High_vector[j]/2)))))
            {
              if (onoff_traffic_mode.compare("Constant") == 0)
              { 
          ...
          {
            if (k < size_t(Num_M_High_vector[j]/2)) // create OnOff machines that generate High priority traffic
            {
              if (onoff_traffic_mode.compare("Constant") == 0)
              { 
                clientHelpers_matrix[j][k].SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(miceOnTime) + "]"));
...
void
viaMQueuesPredictive5ToS_v2 (std::string transport_prot, std::string traffic_control_type, std::string onoff_traffic_mode, double_t mice_elephant_prob, bool accumulate_stats, double_t future_possition, double_t win_length)
{
  LogComponentEnable ("2In2Out", LOG_LEVEL_INFO);
  ...
  std::string usedAlgorythm;
  std::string applicationType = "prioOnOff"; // "standardClient"/"OnOff"/"prioClient"/"prioOnOff"
  std::string transportProt = transport_prot;
  std::string socketType;
  std::string queue_capacity;
  bool eraseOldData = true; // true/false
...
template <size_t N>
void
viaMQueuesPredictive5ToS_v2_VaryingD (std::string transport_prot, std::string traffic_control_type, std::string onoff_traffic_mode, const std::double_t(&mice_elephant_prob_array)[N], double_t alpha_high, double_t alpha_low, bool adjustableAlphas, bool accumulate_stats, double_t future_possition, double_t win_length)
{
  LogComponentEnable ("2In2Out", LOG_LEVEL_INFO);

  // for local d(t) estimation
  // Tau = win_length[sec]. start predictive model at t0 - Tau/2. use window of: [t0 - Tau/2, t0 + Tau/2]
  ...
  for (size_t i = 0; i < N; i++) // iterate over all D values to create SourceAppContainers Array
  {
    ApplicationContainer sourceApps;
    sourceApps_vector.push_back(sourceApps);
    ApplicationContainer sourceAppsPredict;
    sourceAppsPredict_vector.push_back(sourceAppsPredict);
  }
  ...
  std::vector<double_t> Num_M_High_vector;  
  std::vector<bool> unequalNum_vector;  
  for (size_t i = 0; i < N; i++) // iterate over all D values
  {
    double_t Num_M_High = mice_elephant_prob_array[i] * 10;  // number of OnOff machines that generate High priority traffic
    Num_M_High_vector.push_back(Num_M_High);
    bool unequalNum = false;  // a flag that's raised if the number of High/Low priority OnOff applications created is diffeent for each Port
  ...
  for (size_t i = 0; i < 2; i++) // iterate over switch ports
  {
    ...
    std::vector<std::vector<PrioOnOffHelper>> clientHelpers_matrix;
    std::vector<std::vector<PrioOnOffHelper>> clientHelpersPredict_matrix;
    for (size_t d = 0; d < N; d++) // iterate over all D values to configure and install OnOff Application for each D value
    {
      clientHelpers_matrix.push_back(clientHelpers_vector);
      clientHelpersPredict_matrix.push_back(clientHelpersPredict_vector);
      // create and install Client apps:    
      if (applicationType.compare("prioOnOff") == 0) 
      {
        for (size_t k = 0; k < 5; k++) 
        {
          clientHelpers_matrix[d][k].SetAttribute ("Remote", AddressValue (socketAddress_vector[k]));
          // for predicting traffic in queue
          clientHelpersPredict_matrix[d][k].SetAttribute ("Remote", AddressValue (socketAddressPredict_vector[k]));

          if (unequalNum_vector[d]) 
          {
            // in case the number of High/Low priority OnOff applications is not equally devidable by 2, 
            // divide the High/Low priority OnOff applications between the 2 servers such that:
            // 1st server recives the higher number of High priority and lower number of Low priority OnOff applications
            // and the 2nd server recives lower number of High priority and higher number of Low priority OnOff applications
            if (((i == 0) && (k < static_cast<size_t>(ceil(Num_M_High_vector[d]/2)))) || ((i == 1) && (k < static_cast<size_t>(floor(Num_M_High_vector[d]/2)))))
            {
              if (onoff_traffic_mode.compare("Constant") == 0)
              { 
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(miceOnTime) + "]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant="+ DoubleToString(miceOffTime) +"]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(miceOnTime) + "]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant="+ DoubleToString(miceOffTime) +"]"));
              }
              else if (onoff_traffic_mode.compare("Uniform") == 0)
              {
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max="+ DoubleToString(miceOnTimeMax) +"]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max=" + DoubleToString(miceOffTimeMax) + "]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max="+ DoubleToString(miceOnTimeMax) +"]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max=" + DoubleToString(miceOffTimeMax) + "]"));
              }
              else if (onoff_traffic_mode.compare("Normal") == 0)
              {
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::NormalRandomVariable[Mean=" + DoubleToString(miceOnTime) + "|Variance=" + DoubleToString(miceOnTime) + "|Bound=" + DoubleToString(miceOnTime) + "]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::NormalRandomVariable[Mean="+ DoubleToString(miceOffTime) +"|Variance="+ DoubleToString(miceOffTime) +"|Bound="+ DoubleToString(miceOffTime) +"]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::NormalRandomVariable[Mean=" + DoubleToString(miceOnTime) + "|Variance=" + DoubleToString(miceOnTime) + "|Bound=" + DoubleToString(miceOnTime) + "]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::NormalRandomVariable[Mean="+ DoubleToString(miceOffTime) +"|Variance="+ DoubleToString(miceOffTime) +"|Bound="+ DoubleToString(miceOffTime) +"]"));
              }
              else if (onoff_traffic_mode.compare("Exponential") == 0)
              {
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=" + DoubleToString(miceOnTime) + "|Bound=" + DoubleToString(trafficGenDuration) + "]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean="+ DoubleToString(miceOffTime) +"|Bound="+ DoubleToString(trafficGenDuration) +"]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=" + DoubleToString(miceOnTime) + "|Bound=" + DoubleToString(trafficGenDuration) + "]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean="+ DoubleToString(miceOffTime) +"|Bound="+ DoubleToString(trafficGenDuration) +"]"));
              }
              else 
              {
                std::cerr << "unknown OnOffMode type: " << onoff_traffic_mode << std::endl;
            ...
            else
            {
              if (onoff_traffic_mode.compare("Constant") == 0)
              { 
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(elephantOnTime) + "]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(elephantOffTime) + "]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=" + DoubleToString(elephantOnTime) + "]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant="+ DoubleToString(elephantOffTime) +"]"));
              }
              else if (onoff_traffic_mode.compare("Uniform") == 0)
              {
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max="+ DoubleToString(elephantOnTimeMax) +"]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max=" + DoubleToString(elephantOffTimeMax) + "]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max="+ DoubleToString(elephantOnTimeMax) +"]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::UniformRandomVariable[Min=0.|Max=" + DoubleToString(elephantOffTimeMax) + "]"));
              }
              else if (onoff_traffic_mode.compare("Normal") == 0)
              {
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::NormalRandomVariable[Mean=" + DoubleToString(elephantOnTime) + "|Variance=" + DoubleToString(elephantOnTime) + "|Bound=" + DoubleToString(elephantOnTime) + "]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::NormalRandomVariable[Mean="+ DoubleToString(elephantOffTime) +"|Variance="+ DoubleToString(elephantOffTime) +"|Bound="+ DoubleToString(elephantOffTime) +"]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::NormalRandomVariable[Mean=" + DoubleToString(elephantOnTime) + "|Variance=" + DoubleToString(elephantOnTime) + "|Bound=" + DoubleToString(elephantOnTime) + "]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::NormalRandomVariable[Mean="+ DoubleToString(elephantOffTime) +"|Variance="+ DoubleToString(elephantOffTime) +"|Bound="+ DoubleToString(elephantOffTime) +"]"));
              }
              else if (onoff_traffic_mode.compare("Exponential") == 0)
              {
                clientHelpers_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=" + DoubleToString(elephantOnTime) + "|Bound=" + DoubleToString(trafficGenDuration) + "]"));
                clientHelpers_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean="+ DoubleToString(elephantOffTime) +"|Bound="+ DoubleToString(trafficGenDuration) +"]"));
                // for Predictive model
                clientHelpersPredict_matrix[d][k].SetAttribute ("OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=" + DoubleToString(elephantOnTime) + "|Bound=" + DoubleToString(trafficGenDuration) + "]"));
                clientHelpersPredict_matrix[d][k].SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean="+ DoubleToString(elephantOffTime) +"|Bound="+ DoubleToString(trafficGenDuration) +"]"));
              }
              else 
              {
                std::cerr << "unknown OnOffMode type: " << onoff_traffic_mode << std::endl;
          ...
          clientHelpers_matrix[d][k].SetAttribute ("DataRate", StringValue (IntToString(dataRate) + "Mb/s"));
          clientHelpers_matrix[d][k].SetAttribute("StreamIndex", UintegerValue (1 + 2*(i + k))); // assign a stream for RNG for each OnOff application instanse
          sourceApps_vector[d].Add(clientHelpers_matrix[d][k].Install (servers.Get(serverIndex)));

          // for Predictive model
          clientHelpersPredict_matrix[d][k].SetAttribute ("PacketSize", UintegerValue (PACKET_SIZE));
          clientHelpersPredict_matrix[d][k].SetAttribute ("DataRate", StringValue (IntToString(dataRate) + "Mb/s"));
          clientHelpersPredict_matrix[d][k].SetAttribute("StreamIndex", UintegerValue (1 + 2*(i + k))); // assign a stream for RNG for each OnOff application instanse
          sourceAppsPredict_vector[d].Add(clientHelpersPredict_matrix[d][k].Install (serversPredict.Get(serverIndex)));
        }
      }
      else 
      {
  ...
  for (size_t i = 0; i < N; i++) // iterate over all D values to synchronize Start/Stop for all OnOffApps
  {
    sourceApps_vector[i].Start (Seconds (1.0 + i * (trafficGenDuration + silenceDuration)));
    sourceApps_vector[i].Stop (Seconds (1.0 + (i + 1) * trafficGenDuration + i * silenceDuration));
    // for Predictive model: start win_length*future_possition seconds earlier, stop win_length*(1-future_possition) seconds earlier
    sourceAppsPredict_vector[i].Start (Seconds (1.0 + i * (trafficGenDuration + silenceDuration) - win_length * future_possition));
    sourceAppsPredict_vector[i].Stop (Seconds (1.0 + (i + 1) * trafficGenDuration + i * silenceDuration - win_length * (1 - future_possition)));
  }
  ...
  // move all the produced .dat files to a VaryingDValues/Predictive/ directory
  // datDir = "./Trace_Plots/test_Alphas/"
  // dir = datDir + "future_possition/win_length/"
  // dirToSave = dir + traffic_control_type + "/" + implementation + "/" + transportProt + "/" + onoff_traffic_mode + "/";
  std::string newDir;
  if (adjustableAlphas)
  {
    newDir = dirToSave + "VaryingDValues/adjustableAlphas/Predictive/";
  }
  else
  {
    newDir = dirToSave + "VaryingDValues/" + DoubleToString(alpha_high) + "_" + DoubleToString(alpha_low) + "/Predictive/";
  }
  system (("mkdir -p " + newDir).c_str ());
  system (("mv -f " + datDir + "*.dat " + newDir).c_str ());
  system (("mv -f " + dirToSave + "*.txt " + newDir).c_str ());
  ...
  if (accumulate_stats)
  {
    if (!(std::filesystem::exists(dir + "/TestStats/" + traffic_control_type + "/" + implementation + "/" + transportProt + "/" + onoff_traffic_mode + "/VaryingDValues/" 
                                  + usedAlgorythm + "_TestAccumulativeStatistics.dat")))
    {
      // If the file doesn't exist, create it and write initial statistics
      system (("mkdir -p " + dir + "/TestStats/" + traffic_control_type + "/" + implementation + "/" + transportProt + "/" + onoff_traffic_mode + "/VaryingDValues/").c_str ());
      std::ofstream testAccumulativeStats (dir + "/TestStats/" + traffic_control_type + "/" + implementation + "/" + transportProt + "/" + onoff_traffic_mode + "/VaryingDValues/" 
                                            + usedAlgorythm + "_TestAccumulativeStatistics.dat", std::ios::app);
      if (adjustableAlphas)
      {
        testAccumulativeStats
    ...
    {
      // Open the file in append mode
      std::fstream testAccumulativeStats (dir + "/TestStats/" + traffic_control_type + "/" + implementation + "/" + transportProt + "/" + onoff_traffic_mode + "/VaryingDValues/" 
                                          + usedAlgorythm + "_TestAccumulativeStatistics.dat", std::ios::app);
      if (adjustableAlphas)
      {
        testAccumulativeStats
